        ...
        switch (action) {
            case 'on':
                ...
                if (existingConfig?.enabled) {
                    await sock.sendMessage(chatId, { text: '*_Antilink Dev Shadow deja activé_*' }, { quoted: message });
                    return;
                }
                const result = await setAntilink(chatId, 'on', 'delete');
                await sock.sendMessage(chatId, { 
                    text: result ? '*_Antilink Dev Shadow ON_*' : '*_Failed to turn on Antilink_*' 
                },{ quoted: message });
                break;

            case 'off':
                await removeAntilink(chatId, 'on');
                await sock.sendMessage(chatId, { text: '*_Antilink Dev Shadow OFF_*' }, { quoted: message });
                break;
